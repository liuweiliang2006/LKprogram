...
void Master_Send_Alarm_Data(uint8_t *data)
{
  ...
  __disable_irq();//主机发送数据，关总中断
	...
	{
		Usart_SendByte(UART5,ucAlarmData[i]);
	}
	Pack_Num_Count();
...
void Master_Send_GetCarState_data(void)
{
	...
	{
		Usart_SendByte(UART5,ucGetCarStateData[i]);
	}
	Pack_Num_Count();
...
extern uint8_t ucSetThreshold[18]; //主机气馈给从机的域值数据
...
//static uint8_t cmdcount=1;
/*salve recive buff data Analysis */
void UcAnalysis_RecSlave_Data(void)
{
	uint16_t i;
	...
	if((ucSlaveCrcCheckSum&0xff)==ucReciveBuffer[ucSalvePackLen-1])  //判断根据接收到的数据得出的CRC与从机发送的CRC校验是否一致
	{
		...
		{
			case Cmd_Alarm: //报警命令，从机响应主机
			{
				Slave_Response_Master(ucSalvePackLen,Cmd_Alarm);
				break;
			}
			case Cmd_GetCarState: //获取车辆状态命令，从机响应主机
			{		
				g_ucGetCarStateresponse=0;
				memcpy(&g_tCarDoorState,&ucReciveBuffer[7],7);
				if(g_tCarDoorState.DOORBITS.DOOR==0) //获取从机得到车门状态已为关状态
				{
					g_ucCarDoorFlag=CLOSE; //车门状态位清0，表示关闭
				}			
//				Slave_Response_Master(ucSalvePackLen,Cmd_GetCarState);							//接收从机的数据存入指定的BUFFER
				break;
			}
			case Cmd_SendData:  //发送数据命令，从机响应主机
			{
				Slave_Response_Master(ucSalvePackLen,Cmd_SendData);
				break;
			}
			case Cmd_SetThreshold: //设置域值命令，主机响应从机
			{
				ucPackNum=ucReciveBuffer[Package_Posi];
				if(ucReciveBuffer[3]==0x12)
				{
					memcpy(&g_tCarThreshold,&ucReciveBuffer[7],18);
					memcpy(&ucSetThreshold,&g_tCarThreshold,18);
					g_ucThresholdFlag=TRUE; //从机传输设置阈值，置阈值标识符有效。
				}
				
				if(ucReciveBuffer[3]==0x00)
				{
					memcpy(&ucSetThreshold,&g_tCarThreshold,18);
				}				
				Master_Response_Slave(0x12,Cmd_SetThreshold);			
				break;
			}
			case Cmd_Report: //上报命令，主机响应从机
			{
				ucPackNum=ucReciveBuffer[Package_Posi];
				memcpy(&g_tCarState,&ucReciveBuffer[7],6);
				/*检测车门的当前状态与前一时刻的车门状态是否一致*/
				if(g_tCarState.DOORBITS.DOOR!=g_ucCarDoorFlag)
				{
					g_ucGetCarStateresponse=0;
				}
				
				if(g_tCarState.DOORBITS.DOOR==0) 
				{/*从机得到车门状态已为关状态*/
					g_ucCarDoorFlag=CLOSE; //车门状态位清0，表示关闭
				}
